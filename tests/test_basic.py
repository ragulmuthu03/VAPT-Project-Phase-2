...
class TestVAPTOperation(unittest.TestCase):
    ...
    def run_vapt_commands(self, input_commands):
        """Run the VAPT tool interactively with proper arguments and return output."""
        if not shutil.which("python3"):
            self.skipTest("Python3 not found in system path.")

        process = subprocess.Popen(
            ["sudo", "python3", "tidconsole.py", "--quiet"],  # Runs in quiet mode
    ...
    def test_vapt_execution(self):
        """Test if the VAPT tool executes commands successfully."""
        input_commands = (
            "?\n"  # Show help menu
            "list\n"  # List available modules
            "vicadd www.example.com\ny\n\n"  # Add victim with SSL confirmation
        ...
        expected_keywords = ["help", "list", "victims", "attack", "geoip", "country", "latitude", "longitude"]
        self.assertTrue(
